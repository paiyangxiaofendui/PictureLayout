...
int ALGORITHM_API::New_LayoutOnePanel_Greedy(Panel* pSrcPanel, BaseInfo& Info, vector<Component*>& SrcComponentList, int CutStyle, int Org)
{
	...
	while (SrcComponentList.size() > 0)
	{
		...
		if (bMatchable == true)
		{
			...
			// 排样
			if (Info.m_AutoSpace == 1 && Info.m_OneLabel == 1 && pPerfectMatchComponent->m_IndexInSameCpn == 0)
			{
				ALGORITHM_API::New_KnifeOneRemainder_AutoSpace(pPerfectMatchRemainder, pPerfectMatchComponent, nRecommendCutDir, Org, Info);
...
bool ALGORITHM_API::New_KnifeOneRemainder_AutoSpace(Component* pParentNode, Component* pPlaceCpn, int CutDir,  int Org, BaseInfo& b_info)
{
	// 判断要切割的小板是否合法
	if (pPlaceCpn->IsLegal() == false)
	{
	...
	if (CutDir == CutDir_Vertical)
	{
		...
		case LayoutOrg_LeftBottom:		// 左下角
			...
			pSecondRight->m_y			= pLeft->m_y + pPlaceCpn->m_RealWidth + y_space;				// 父节点的y + 小板宽度 + 锯缝
			pSecondRight->m_RealLength	= pLeft->m_RealLength;											// 父节点的长
			pSecondRight->m_RealWidth	= pLeft->m_RealWidth - pPlaceCpn->m_RealWidth - y_space;	// 父节点宽度 - 小板宽度 - 锯缝
		...
		case LayoutOrg_RightBottom:		// 右下角
			...
			// 另一块余料
			pSecondRight->m_x			= pLeft->m_x;													// 父节点的x
			pSecondRight->m_y			= pLeft->m_y  + pPlaceCpn->m_RealWidth + y_space;			// 父节点的y + 小板的宽度 + 锯缝
			pSecondRight->m_RealLength	= pLeft->m_RealLength;											// 父节点的长
			pSecondRight->m_RealWidth	= pLeft->m_RealWidth - pPlaceCpn->m_RealWidth - y_space;	// 父节点宽度 - 小板宽度 - 锯缝
			pSecondRight->m_Thickness	= pLeft->m_Thickness;
			pSecondRight->m_Texture		= pLeft->m_Texture;
		...
		default:						// 默认左下角
			...
			pSecondRight->m_y			= pLeft->m_y + pPlaceCpn->m_RealWidth + y_space;		// 父节点左下角 + 小板宽度 + 锯缝
